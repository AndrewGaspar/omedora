...
> began. Everything reusable from it already landed, refined, on `omedora-3` — the only
> dev-only extras were L4 golden-image tests targeting walker/waybar, which Omarchy 4
> retires. **All porting here sources from `omedora-3`; do not mine origin/dev.**
...
No third-party COPR is enabled here. Task #66 vendored the entire hyprwm stack as omedora RPMs (`omedora/packaging/copr/`), built from omedora's own COPR and served from the omedora repo; the base system enables no third-party COPR. (The one allowed third-party COPR, `scottames/ghostty`, is enabled lazily elsewhere, only if a user opts into the Ghostty terminal.)
...
| `omedora/packaging/copr/terminaltexteffects.spec` | New | Omedora 3 only | From-source Python package retired from the Omedora 4 build set; Quattro uses native `ttfx` instead |
| `omedora/packaging/copr/{omacalc,ttfx}.spec` | New | ✅ shipped | Quattro calculator and native terminal-effects engine; omacalc is Qt6/qmake, ttfx is hermetic Cargo and obsoletes `python3-terminaltexteffects` |
| `omedora/packaging/copr/herdr.spec` | New | ✅ shipped | Omarchy's pinned terminal-workspace fork; hermetic Cargo build with upstream's exact build-only Zig 0.15.2 and a prefilled offline Zig dependency cache |
...
| `omedora/packaging/copr/hyprland.spec` | New | ✅ shipped | **The compositor (task #66).** Built from the release `source-v0.55.2.tar.gz` (bundles udis86 + hyprland-protocols subprojects). Ships `macros.hyprland` for hyprpm. Built from omedora's own COPR. |
...
| `omedora/packaging/copr/voxtype.spec` (+ `.sources`) | New | ✅ shipped | **Subpackaged binary-repackage** of upstream's official Fedora RPM (`Source0` = the prebuilt `.rpm`; `.sources` pins its sha256). One spec → three packages: base `voxtype` (CPU+Vulkan+ONNX-CPU, `Requires: curl pipewire-alsa vulkan-loader`) + opt-in `voxtype-cuda` / `voxtype-migraphx` (each `Requires: voxtype = %{version}-%{release}`, `AutoReqProv: no`). No `%build`; `%install` `rpm2cpio`-extracts and `%files` carves the split. `QA_RPATHS` exported in `%install` to allow upstream's build-machine runpaths on the GPU provider libs. On-demand only; not in `build-repo.sh`'s default base path but listed in its `SPECS` array. Bump = `Version:` + `.sources` sha |
...
| `bin/fedora/voxtype-install-pkg` | New | ✅ shipped | Fedora arm of `omarchy-voxtype-install`: the **flavor installer**. Installs the slim base (`omarchy-pkg-add wtype voxtype-bin` → the `voxtype` RPM from our COPR), then **auto-detects the GPU** and adds the matching add-on — `voxtype-cuda` on `omarchy-hw-nvidia`, `voxtype-migraphx` on an AMD `lspci` match, base-only otherwise. All via `omarchy-pkg-add` (honors `$OMARCHY_PKG_DRY_RUN` in `pkg.py`). No download/sha here — COPR's SRPM step fetches + sha-verifies upstream; bump = `Version:` + `.sources` sha in `voxtype.spec` |
...
| `install/preflight/fedora-repos.sh` | New | ✅ shipped | RPM Fusion + Flathub remote enable. No third-party COPR is enabled here — task #66 vendored the hyprwm stack under `omedora/packaging/copr/`, built from omedora's own COPR and served from the omedora repo |
