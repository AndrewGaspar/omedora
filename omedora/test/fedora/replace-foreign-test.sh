...
#
#   1. Install a real FOREIGN Hyprland from a third-party COPR (solopasha/hyprland
#      by default; override with REPLACE_FOREIGN_COPR) — a genuine third-party
#      `hyprland` that owns /usr/bin/Hyprland.
...
# An example third-party Hyprland COPR, used only to stand in for "some foreign
# hyprland" so the replacement engine has something to evict. Override via env.
