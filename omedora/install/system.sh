...
# omarchy-setup-system the Arch ISO calls in the target chroot. On Fedora the
...
echo -e "\n\e[32mOmedora: applying system setup (omarchy-setup-system)\e[0m"

setup_env=(OMARCHY_LOG_TO_STDOUT="${OMARCHY_LOG_TO_STDOUT:-1}")
setup_cmd=(omarchy-setup-system)
if [[ -n ${OMEDORA_SETUP_FROM_REPO:-} ]]; then
  ...
  setup_cmd=("$OMEDORA_REPO_ROOT/bin/omarchy-setup-system")
fi
