# Run the root-owned system setup from the INSTALLED payload — `omarchy apply
# system`, which the Arch ISO calls in the target chroot (it also drives
# omarchy-apply-hardware, so hardware setup cannot be skipped). On Fedora the
# Arch-only scripts inside it are gated (login/sddm, post-install/pacman,
# pam edits return early) and the rest dispatch to -fedora siblings
# (firewalld, service enables, docker daemon.json only-if-absent). See
# omedora/architecture.md §"Omarchy 4 setup-system gating map" for the
# per-file decisions.
#
