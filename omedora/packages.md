...
| `voxtype` / `voxtype-bin` | **on-demand, vendored from upstream's official Fedora RPM** | Not in `omarchy-base.packages` — installed on demand by `omarchy-voxtype-install`. On Fedora its arm runs `bin/fedora/voxtype-install-pkg`, which downloads voxtype's **official upstream Fedora RPM** (`peteonrails/voxtype` release asset, Name: `voxtype`), pinned to **v0.7.5** + sha256 `50dd10fe…79c61`, verifies the sha (aborts on mismatch — the supply-chain gate), then `dnf install`s it. The RPM bundles every binary variant under `/usr/lib/voxtype/` with a `/usr/bin/voxtype` wrapper, so `voxtype setup --download / gpu --enable / systemd` run unchanged. `voxtype-bin` (the Arch package) stays `skip` in `fedora.toml` because it never reaches `pkg.py` on Fedora — the arm installs the RPM directly. `wtype` → dnf (native); best-effort `vulkan-loader` only when `omarchy-hw-vulkan` succeeds. **Bump = edit the two pinned constants** (`VOXTYPE_VERSION`, `VOXTYPE_RPM_SHA256`) in `bin/fedora/voxtype-install-pkg` (download the new `.rpm`, `sha256sum` it). Follow-up hardening: upstream also GPG-signs `…rpm.asc` / `SHA256SUMS.txt.asc` — sha256 is the required gate; `.asc` verification is a nice-to-have. |
