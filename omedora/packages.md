...
| **From-source (Python pyproject)** | A Python project | `terminaltexteffects.spec` (Omedora 3 only) | Uses Fedora's `pyproject-rpm-macros`: `%pyproject_buildrequires` derives build deps, `%pyproject_wheel`/`%pyproject_install`/`%pyproject_save_files` build and capture the wheel + console scripts (`tte`). Quattro replaces this package with native `ttfx`; the old spec remains only as Omedora 3 history. |
...
| `build-repo.sh` | Builds the canonical spec set in dependency order by looping `build-local.sh`, adding each result to an incremental local dnf repo so later builds can consume sibling RPMs. The Omedora 4 set includes the Hyprland/HypXRland waves and Quattro additions such as omacalc, ttfx, and herdr. |
...
The from-source Rust specs (`swayosd.spec`, `satty.spec`, `tensaku.spec`, `bluetui.spec`, `ttfx.spec`, and `herdr.spec`) build **fully offline**: COPR builds in mock, where the rpmbuild (build) phase has no network — only SRPM generation does. So cargo must never reach crates.io at build time. Each spec ships a `cargo vendor` tarball as an extra `Source` and a `.cargo/config.toml` with `[net] offline = true` (written by `%cargo_prep`, or by hand for the meson-driven swayosd), so the build resolves every crate from the vendored tree. Herdr additionally ships a generated Zig global-cache source because its embedded Ghostty terminal engine uses Zig's package manager.
...
| `voxtype` / `voxtype-bin` | **on-demand, from-source build hosted in `agaspar/omedora-4`** | Not in `omarchy-base.packages` — installed on demand by `omarchy-voxtype-install`. We build the MIT-licensed AndrewGaspar/voxtype fork (branch feat/muse-stack-v1.0.1: v1.0.1 + Muse streaming-transcribe engine + OSD states) from source in our COPR via **one spec → one package** (`omedora/packaging/copr/voxtype.spec`): **`voxtype`** (tiered whisper avx2/avx512/vulkan binaries under `/usr/lib/voxtype/` with a `%post`-managed `/usr/bin/voxtype` symlink + OSD helpers + quickshell QML + config + user service + man pages; `Requires: curl pipewire-alsa vulkan-loader`). CPU-only dep set — the old `voxtype-cuda` / `voxtype-migraphx` ONNX-GPU subpackages are gone (ort prebuilts + CUDA/ROCm toolchains aren't available to offline COPR builds); GPU users keep the Vulkan tier. On Fedora, `bin/fedora/voxtype-install-pkg` installs the base (`omarchy-pkg-add wtype voxtype-bin`, mapped to `voxtype`) and nothing else, and the whole thing is **`dnf update`-able**. `voxtype-bin` is mapped `source = "dnf"` in `fedora.toml`. `voxtype setup --download / gpu --enable / systemd` run unchanged. **Bump = edit `Version:` / `%global fork_commit` + the `.sources` sha** in the spec (COPR's SRPM step fetches + sha-verifies the pinned fork archive; build-local.sh regenerates the cargo-vendor tarball deterministically). |
