...
#     applies it (that's where omarchy puts --ozone-platform=wayland, the
#     copy-url extension, etc.). Fedora's chromium-browser is a plain binary
#     that ignores the file — so chromium falls back to X11, needs $DISPLAY,
#     and fails to start in a Wayland session.
#
...
if ! command -v chromium-browser >/dev/null 2>&1; then
  echo "chromium-browser not installed; skipping Chromium bridge."
...
# resolve it regardless of their PATH). This is what omarchy's launch scripts
# read the Exec from, and what the launcher shows.
...
# (ours, Wayland-enabled) rather than two — the system entry would launch
# /usr/bin/chromium-browser directly, bypassing the flags, and fail on Wayland.
